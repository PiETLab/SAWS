 ...
 * 
 */
public class ShowForElijah extends
	...
	public ShowForElijah(TraversableEncodingTree encodingTree,
	...
	public void updateAppearance() {
		...
		if (canFitOnDisplay(getEncodingTree().getCurrentSelectionGroupParent())) {
			// showInFokus(sg);
	...
	private boolean canFitOnDisplay(SelectionGroup sg) {
		if (IS_VERBOSE)
			System.out.println("can fit on display this sg?"
					+ sg.extractMembers().size()
					+ "\t given max allowable elements: "
	 ...
	 * @param sg
	 */
	private void showInFokus(SelectionGroup sg) {
		JPanel panel = new JPanel();
		panel.setLayout(new BoxLayout(panel, BoxLayout.X_AXIS));
		panel.setAlignmentX(Component.LEFT_ALIGNMENT);
		panel.setBackground(super.paramModel.getBackgroundColour());
	...
	private void versionLeaveInPositionAndHighlight(SelectionGroup sg) {
		// it may be the case that the focus cycle has not yet been drawn on the
		// screen (e.g., if the leaf is the child of the root
		// boolean isAlreadyDrawn = getOnScreenKeyboardRepresentation()
		// .getComponentCount() != 0;
		// if (!isAlreadyDrawn) {
		resetToDefault(getEncodingTree().getCurrentSelectionGroupParent());
		showInFokus(getEncodingTree().getCurrentSelectionGroupParent());
		// }
		...
		for (SourceSymbol x : sg.extractMembers()) {
			...
			// if (but.getIcon()!= null );but.setIcon(img);
			if (IS_VERBOSE) {
	...
	public Font getFontForLeaves() {
		// return super.getFont().deriveFont(460f);
