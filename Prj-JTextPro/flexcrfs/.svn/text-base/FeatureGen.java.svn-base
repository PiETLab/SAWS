...
public class FeatureGen implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3405168501872926392L;

	List<Feature> features = null; // list of features

	Map<String, Integer> fmap = null; // feature map
		...
		for (int i = 0; i < obsr.cps.length; i++) {
			...
			if (!(elem.isScanned)) {
				...
				while (it.hasNext()) {
					...
					CountFeatureIdx cntFidx = (CountFeatureIdx) elem.lbCntFidxes
							.get(label);
