...
public class RamayKeyboardLayoutModified extends ButtonLayoutSpecification 
{
...
//	private static final int H = 2;
	...
	public RamayKeyboardLayoutModified(MyTraversableEncodingTree<JIndirectSelectionButton> root, SourceSymbolSet selectableKeyList) 
	{
		...
		Graph.constructGraph(allButtons, "Modified_Technique3_Aug29.txt");
	}
	
	public KBLNode<JIndirectSelectionButton> preCalculation1 (KBLNode<JIndirectSelectionButton> parentLayoutNode)
	{
	...
	public KBLNode<JIndirectSelectionButton> preCalculation2 (KBLNode<JIndirectSelectionButton> parentLayoutNode)
	{
	...
	public KBLNode<JIndirectSelectionButton> preCalculation3 (KBLNode<JIndirectSelectionButton> parentLayoutNode)
	{
		...
		float minWidthPercentage = (MIN_BUTTON_WIDTH/(float)SCREEN_WIDTH)* MAX_PERCENTAGE;
		float minHeightPercentage = (MIN_BUTTON_HEIGHT/(float)SCREEN_HEIGHT)* MAX_PERCENTAGE;
	...
	public KBLNode<JIndirectSelectionButton> preKBLCalculation3 (KBLNode<JIndirectSelectionButton> parentLayoutNode)
	{
		Node<JIndirectSelectionButton> parentNode = parentLayoutNode.getNode();
		List <Node<JIndirectSelectionButton>> children = parentNode.getChildren();
		...
		else
		{
			if (parentLayoutNode.getDivideType() == KBLNode.VERTICAL)
			{
				...
				for (int i = 0; i < children.size(); i++)
				{
					...
					KBLNode<JIndirectSelectionButton> tmpChildNode = preKBLCalculation3(newChildNode);
					if(i == 0)
					{
	...
	public KBLNode<JIndirectSelectionButton> preKBLCalculation2 (KBLNode<JIndirectSelectionButton> parentLayoutNode, int level, int maxLevel)
	{
		Node<JIndirectSelectionButton> parentNode = parentLayoutNode.getNode();
		List <Node<JIndirectSelectionButton>> children = parentNode.getChildren();
			...
			if (parentLayoutNode.getDivideType() == KBLNode.VERTICAL)
			{
				...
				for (int i = 0; i < children.size(); i++)
				{
					...
					if(parentLayoutNode.getAvaialbleWidth() != -1)
					{
						...
						//level = level +1;
						KBLNode<JIndirectSelectionButton> tmpChildNode = preKBLCalculation2(newChildNode,level, maxLevel);
						if(i == 0)
						{
							parentLayoutNode.setRequiredWidth(tmpChildNode.getRequiredWidth());
							//parentLayoutNode.setRequiredHeight(tmpChildNode.getRequiredHeight());
	...
	public KBLNode<JIndirectSelectionButton> preKBLCalculation1 (KBLNode<JIndirectSelectionButton> parentLayoutNode)
	{
		Node<JIndirectSelectionButton> parentNode = parentLayoutNode.getNode();
		List <Node<JIndirectSelectionButton>> children = parentNode.getChildren();
			...
			if (parentLayoutNode.getDivideType() == KBLNode.VERTICAL)
			{
				...
				for (int i = 0; i < children.size(); i++)
				{
					...
					if(parentLayoutNode.getAvaialbleWidth() != -1)
					{
						...
						newChildNode.setAvaialbleHeight(parentLayoutNode.getAvaialbleHeight());
						//level = level +1;
						KBLNode<JIndirectSelectionButton> tmpChildNode = preKBLCalculation1(newChildNode);
						if(i == 0)
						{
							parentLayoutNode.setRequiredWidth(tmpChildNode.getRequiredWidth());
							//parentLayoutNode.setRequiredHeight(tmpChildNode.getRequiredHeight());
		...
		else
		{
			if (parentLayoutNode.getDivideType() == KBLNode.VERTICAL)
			{
				...
				for (int i = 0; i < kblChildren.size(); i++)
				{
					...
					//divide type is set. 
					setChildLayoutNode(parentLayoutNode,kblChild, KBLNode.WIDTH,interval, i,KBLNode.HORIZONTAL );
			...
			else
			{
				...
				for (int i = 0; i < kblChildren.size(); i++)
				{
					...
					//New child node is created and dimensions are assigned to it accordingly, and the 
					//divide type is set. 
					setChildLayoutNode(parentLayoutNode,kblChild, KBLNode.HEIGHT,interval, i, KBLNode.VERTICAL );				
					JPanel subBoxContainer = CHToKBL (kblChild, technique);
					boxContainer.add(subBoxContainer);
					this.configureBoxContainer(boxContainer, parentLayoutNode);
				}
	...
	*/
	...
	private List<Float> scalingTechnique3(KBLNode<JIndirectSelectionButton> parentLayoutNode, int flag, int technique)
	{
		List <KBLNode<JIndirectSelectionButton>> kblChildren = parentLayoutNode.getChildren();
		List <Node<JIndirectSelectionButton>> children = parentLayoutNode.getNode().getChildren();
		...
		{
			KBLNode<JIndirectSelectionButton> kblChild = kblChildren.get(i);
			float tmpChildDim =kblChild.getRequiredDimension(flag)/percentageSum;
