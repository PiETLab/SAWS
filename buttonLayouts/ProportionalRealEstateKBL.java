...
import keyboardLayouts.toBeFixed.KBLNode;
...
public class ProportionalRealEstateKBL extends ButtonLayoutSpecification {
	...
	public ProportionalRealEstateKBL(TraversableEncodingTree root,
			SourceSymbolSet selectableKeyList) {
		keyList = selectableKeyList;

		KBLNode rootNode = new KBLNode(root.getRoot());
		...
		KBLNode tmpRoot = new KBLNode(root.getRoot());

		if (technique == TECHNIQUE1) {
			tmpRoot = preCalculation1(rootNode);
		} else if (technique == TECHNIQUE2) {
			tmpRoot = preCalculation2(rootNode);
		} else if (technique == TECHNIQUE3) {
			tmpRoot = preCalculation3(rootNode);
		}
		JPanel keyboard = CHToKBL(tmpRoot, technique);
		onScreenButtonRows = new Vector<JPanel>();
		onScreenButtonRows.add(keyboard);
		Graph.constructGraph(allButtons, "PropotionalKBL_Sep1.txt");
		Graph.constructGraphProportional(allButtons,
				"PropotionalGraph_PKBL_Sep1.txt", SCREEN_WIDTH, SCREEN_HEIGHT);
		Graph.constructGraphNormalized(allButtons,
				"NormalizedGraph_PKBL_Sep1.txt", SCREEN_WIDTH, SCREEN_HEIGHT);
	}

	public KBLNode preCalculation1(KBLNode parentLayoutNode) {
		parentLayoutNode.setAvaialbleWidth(MAX_PERCENTAGE);
		parentLayoutNode.setAvaialbleHeight(MAX_PERCENTAGE);

		return preKBLCalculation1(parentLayoutNode);

	}

	public KBLNode preCalculation2(KBLNode parentLayoutNode) {
		...
		if (maxLevel != -1) {
			...
			for (j = maxLevel; j >= 1; j--) {
				minWidth = MIN_BUTTON_WIDTH
						* (float) Math.pow(k, Math.ceil((double) j / k));
				minHeight = MIN_BUTTON_HEIGHT
						* (float) Math.pow(k, Math.ceil((double) j / k));
				minHeight = minHeight / 2;

				if (minWidth > SCREEN_WIDTH || minHeight > SCREEN_HEIGHT) {
					// It means the displayed button size for the deepest
					// buttons
					// would be less than the Minimum dimensions specified.
					List<Node> leafNodes = ((InternalNode) parentNode)
							.getleavesAtKBLLevel(j);
		...
		if (smallerLeafNodeSum > 0) {
			...
			// List<Node<JVirtualKeyboardButton>> leafNodes =
			// ((InternalNode<JVirtualKeyboardButton>)parentNode).getleavesAtKBLLevel(maxLevel);

			// this is the maximum width and height which would be consumed by
			// all the leaf nodes
			...
			float leafWidthPercentage = estimatedLeafWidth / SCREEN_WIDTH
					* MAX_PERCENTAGE;
			float leafHeightPercentage = estimatedLeafHeight / SCREEN_HEIGHT
					* MAX_PERCENTAGE;

			float availableWidthPercentage = MAX_PERCENTAGE
					- leafWidthPercentage;
			float availbleHeightPercentage = MAX_PERCENTAGE
					- leafHeightPercentage;
			;

			parentLayoutNode.setAvaialbleWidth(availableWidthPercentage);
			parentLayoutNode.setAvaialbleHeight(availbleHeightPercentage);
			int level = 0;
			return preKBLCalculation2(parentLayoutNode, level, maxLevel);
	...
	public KBLNode preCalculation3(KBLNode parentLayoutNode) {
		// Here level means the depth, and two child nodes can be at different
		// depths,
		...
		float parentHeight = parentLayoutNode.getEndY()
				- parentLayoutNode.getStartY();
		// int maxLevel = parentNode.calculateMaximumLeafLevel();
		int maxDepth = ((InternalNode) parentNode).calculateMaximumLeafDepth(0);
		// This is a list that determines the size of the button by each level.
		for (int i = 0; i <= maxDepth; i++) {
			int num = (((InternalNode) parentNode).getleavesAtLevel(i)).size();
		...
		// float scalingFactorW = (float)k/(float)4;

		float minWidthPercentage = (MIN_BUTTON_WIDTH / (float) SCREEN_WIDTH)
				* MAX_PERCENTAGE;
		float minHeightPercentage = (MIN_BUTTON_HEIGHT / (float) SCREEN_HEIGHT)
				* MAX_PERCENTAGE;

		float scalingFactorW = minWidthPercentage;
		float scalingFactorH = minHeightPercentage;
		...
		while (kontinue) {
			...
			for (int i = 0; i < numOfNodesAtDepth.size(); i++) {
				...
				minWidthPercentage = minWidthPercentage + scalingFactorW;
				minHeightPercentage = minHeightPercentage + scalingFactorH;
				// minWidthPercentage += minWidthPercentage;
				// minHeightPercentage += minHeightPercentage;
				count--;
			}
			if (widthPercentageByLevel.get(0) > MAX_PERCENTAGE / 2
					|| heightPercentageByLevel.get(0) > MAX_PERCENTAGE / 2) {
				kontinue = true;
				minWidthPercentage = (MIN_BUTTON_WIDTH / (float) SCREEN_WIDTH)
						* MAX_PERCENTAGE;
				minHeightPercentage = (MIN_BUTTON_HEIGHT / (float) SCREEN_HEIGHT)
						* MAX_PERCENTAGE;
				scalingFactorW = minWidthPercentage / (float) 2;
				scalingFactorH = minHeightPercentage / (float) 2;
		...
		parentLayoutNode.setRequiredHeight(MAX_PERCENTAGE);
		parentLayoutNode.setRequiredWidth(MAX_PERCENTAGE);
		parentLayoutNode.setAvaialbleHeight(MAX_PERCENTAGE);
		parentLayoutNode.setAvaialbleWidth(MAX_PERCENTAGE);

		return preKBLCalculation3(parentLayoutNode);
	}

	public KBLNode preKBLCalculation3(KBLNode parentLayoutNode) {
		Node parentNode = parentLayoutNode.getNode();
		List<Node> children = parentNode.getChildren();
		...
		if (parentNode.isLeaf()) {
			...
			if (parentLayoutNode.getDivideType() == KBLNode.VERTICAL) {
				float maxHeight = 0;
				// All the children are assigned the avalaible percentages of
				// the width
				for (int i = 0; i < children.size(); i++) {
					Node child = children.get(i);
					KBLNode newChildNode = new KBLNode(child);
					newChildNode.setDivideType(KBLNode.HORIZONTAL);
					KBLNode tmpChildNode = preKBLCalculation3(newChildNode);
					if (i == 0) {
					...
					if (maxHeight < tmpChildNode.getRequiredHeight()) {
						maxHeight = tmpChildNode.getRequiredHeight();
					}
				...
				for (int i = 0; i < children.size(); i++) {
					...
					if (maxWidth < tmpChildNode.getRequiredHeight()) {
						maxWidth = tmpChildNode.getRequiredWidth();
					}
	...
	public KBLNode preKBLCalculation2(KBLNode parentLayoutNode, int level,
			int maxLevel) {
		Node parentNode = parentLayoutNode.getNode();
		List<Node> children = parentNode.getChildren();
			...
			if (parentLayoutNode.getDivideType() == KBLNode.VERTICAL) {
				...
				for (int i = 0; i < children.size(); i++) {
					...
					if (parentLayoutNode.getAvaialbleWidth() != -1) {
						float childWidth = (float) childPercentage
								* parentLayoutNode.getAvaialbleWidth();
						...
						newChildNode.setAvaialbleHeight(parentLayoutNode
								.getAvaialbleHeight());
						// level = level +1;
						KBLNode tmpChildNode = preKBLCalculation2(newChildNode,
								level, maxLevel);
						if (i == 0) {
							parentLayoutNode.setRequiredWidth(tmpChildNode
									.getRequiredWidth());
				...
				for (int i = 0; i < children.size(); i++) {
					...
					if (parentLayoutNode.getAvaialbleHeight() != -1) {
						float childHeight = (float) childPercentage
								* parentLayoutNode.getAvaialbleHeight();

						newChildNode.setDivideType(KBLNode.VERTICAL);
						newChildNode.setAvaialbleHeight(childHeight);
						newChildNode.setAvaialbleWidth(parentLayoutNode
								.getAvaialbleWidth());
						KBLNode tmpChildNode = preKBLCalculation2(newChildNode,
								level, maxLevel);
						if (i == 0) {
	...
	public KBLNode preKBLCalculation1(KBLNode parentLayoutNode) {
		Node parentNode = parentLayoutNode.getNode();
		List<Node> children = parentNode.getChildren();
				...
				for (int i = 0; i < children.size(); i++) {
					...
					if (parentLayoutNode.getAvaialbleWidth() != -1) {
						...
						newChildNode.setAvaialbleHeight(parentLayoutNode
								.getAvaialbleHeight());
						// level = level +1;
						KBLNode tmpChildNode = preKBLCalculation1(newChildNode);
						if (i == 0) {
							parentLayoutNode.setRequiredWidth(tmpChildNode
									.getRequiredWidth());
							// parentLayoutNode.setRequiredHeight(tmpChildNode.getRequiredHeight());
	...
	public JPanel CHToKBL(KBLNode parentLayoutNode, int technique) {
		Node parentNode = parentLayoutNode.getNode();
		List<Node> children = parentNode.getChildren();
		...
		if (parentNode.isLeaf()) {
			...
			if (parentLayoutNode.getDivideType() == KBLNode.VERTICAL) {
				...
				List<KBLNode> kblChildren = parentLayoutNode.getChildren();
				// All the children are assigned the percentages of the width
				List<Float> childWidthPercentage = calculateDimensionPercentage(
						parentLayoutNode, KBLNode.WIDTH, technique);
				for (int i = 0; i < kblChildren.size(); i++) {
					KBLNode kblChild = kblChildren.get(i);
					double nodeSizePercentage = childWidthPercentage.get(i);
					// System.out.println("Width: calc : " +
					// nodeSizePercentage*parentWidth + " requ: "
					// +(kblChild.getRequiredWidth()/100)*SCREEN_WIDTH);
					// System.out.println(" At" +
					// kblChild.getNode().toString());
					// KBLNode<JVirtualKeyboardButton> newChildNode = new
					// KBLNode<JVirtualKeyboardButton>(child);
					float parentWidth = parentLayoutNode.getEndX()
							- parentLayoutNode.getStartX();
					float interval = 0;
					...
					setChildLayoutNode(parentLayoutNode, kblChild,
							KBLNode.WIDTH, interval, i, KBLNode.HORIZONTAL);
				...
				// All the children are assigned the percentages of the width
				// */
				List<Float> childHeightPercentage = calculateDimensionPercentage(
						parentLayoutNode, KBLNode.HEIGHT, technique);
				for (int i = 0; i < kblChildren.size(); i++) {
					KBLNode kblChild = kblChildren.get(i);
					...
					// divide type is set.
					setChildLayoutNode(parentLayoutNode, kblChild,
							KBLNode.HEIGHT, interval, i, KBLNode.VERTICAL);
					JPanel subBoxContainer = CHToKBL(kblChild, technique);
					boxContainer.add(subBoxContainer);
					this.configureBoxContainer(boxContainer, parentLayoutNode);
				}
	...
	private void setChildLayoutNode(KBLNode parentLayoutNode, KBLNode kblChild,
			int flag, float interval, int i, int divideType) {
	...
	private List<Float> scalingTechnique1(KBLNode parentLayoutNode) {
		List<KBLNode> kblChildren = parentLayoutNode.getChildren();
		List<Node> children = parentLayoutNode.getNode().getChildren();
	...
	private List<Float> calculateDimensionPercentage(KBLNode parentLayoutNode,
			int flag, int technique) {
		if (technique == TECHNIQUE1) {
			...
			// return scalingTechnique2Or3(parentLayoutNode, flag,technique);
			return scalingTechnique3(parentLayoutNode, flag, technique);
		} else {
	...
	/*
	 * private List<Float> scalingTechnique2(KBLNode<JVirtualKeyboardButton>
	 * parentLayoutNode, int flag) { List <KBLNode<JVirtualKeyboardButton>>
	 * kblChildren = parentLayoutNode.getChildren(); List <Node<JVirtualKeyboardButton>>
	 * children = parentLayoutNode.getNode().getChildren();
	 * 
	 * int k = children.size(); //calculate the width of the parent, i.e. the
	 * avaialble share of real estate //width for all the children float
	 * parentWidth = parentLayoutNode.getEndX()- parentLayoutNode.getStartX();
	 * float parentHeight = parentLayoutNode.getEndY()-
	 * parentLayoutNode.getStartY(); //Vector<Double> childPercentage = new
	 * Vector<Double>();
	 * 
	 * float parentDimension = 0; int parentScreenDim = 0;
	 * 
	 * if(flag == KBLNode.WIDTH) { parentDimension = parentWidth;
	 * parentScreenDim = SCREEN_WIDTH; } else if (flag == KBLNode.HEIGHT) {
	 * parentDimension = parentHeight; parentScreenDim = SCREEN_HEIGHT; }
	 * 
	 * Vector<Float> childDim = new Vector<Float>(); Vector<Float>
	 * childDimPercentage = new Vector<Float>(); //Vector<Float>
	 * childHeightPercentage = new Vector<Float>();
	 * 
	 * for (int i = 0; i < kblChildren.size(); i++) { KBLNode<JVirtualKeyboardButton>
	 * kblChild = kblChildren.get(i); float tmpChildDim =
	 * kblChild.getRequiredDimension(flag) /MAX_PERCENTAGE * parentScreenDim;
	 * childDim.add(tmpChildDim);
	 * childDimPercentage.add((tmpChildDim/parentDimension)* MAX_PERCENTAGE ); }
	 * 
	 * double percentageSum = 0.0; //Sum all the percentage width values to
	 * check if the total percentage is greater than 100% or less, or //equal
	 * for (int i = 0; i < childDimPercentage.size(); i++) { percentageSum +=
	 * childDimPercentage.get(i); } //calculate the percentage difference double
	 * percentageDiff = (MAX_PERCENTAGE-percentageSum); Vector<Double>
	 * childDiff = new Vector<Double>(); double sumDiff = 0.0;
	 * if(percentageDiff > 0) { sumDiff = 0.0; for (int i = 0; i <
	 * childDimPercentage.size(); i++) { double idealPercentage = 1.0/k; if(
	 * (childDimPercentage.get(i) ) < idealPercentage*MAX_PERCENTAGE) { double
	 * diff = idealPercentage*MAX_PERCENTAGE - childDimPercentage.get(i) ;
	 * sumDiff += diff; childDiff.add(diff); } else { childDiff.add(0.0); } }
	 * for (int i = 0; i < childDiff.size(); i++) { double tmp =
	 * childDiff.get(i) / sumDiff ; childDiff.set(i,tmp); } } else
	 * if(percentageDiff < 0) { sumDiff = 0.0; for (int i = 0; i <
	 * childDimPercentage.size(); i++) { double idealPercentage = 1.0/k; if(
	 * (childDimPercentage.get(i) *MAX_PERCENTAGE) >
	 * idealPercentage*MAX_PERCENTAGE) { double diff = idealPercentage*100 -
	 * (childDimPercentage.get(i) *MAX_PERCENTAGE) ; sumDiff += Math.abs(diff);
	 * childDiff.add(diff); } else { childDiff.add(0.0); } } //We calcualte what
	 * is the percentage that should be subtracted from each child's width //
	 * percentage to make sure the total width percentage remains == 100, i.e
	 * the child width //sum up tp the width of the parent, and is not more that
	 * it.
	 * 
	 * for (int i = 0; i < childDiff.size(); i++) { double tmp =
	 * childDiff.get(i) / sumDiff ; childDiff.set(i,tmp); } }
	 * 
	 * else { for (int i = 0; i < childDimPercentage.size(); i++) {
	 * childDiff.add(0.0); } }
	 * 
	 * //All the above lines of code just calculated the percentage of width
	 * which each child should be given, //in order to make sure that each
	 * button would be visible on screen, and no button would have width //
	 * smaller than Min-width. for (int i = 0; i < childDimPercentage.size();
	 * i++) { double tmp = ( childDimPercentage.get(i) + (
	 * Math.abs(percentageDiff) * childDiff.get(i) )) / MAX_PERCENTAGE;
	 * childDimPercentage.set(i, (float)tmp); } return childDimPercentage; }
	 */
	/*
	...
	private List<Float> getIdealPercentageRatio(KBLNode parentLayoutNode,
			int technique) {
	...
	private List<Float> getPercentageRatioTechnique2(KBLNode parentLayoutNode) {
		List<Float> childDimRatio = new Vector<Float>();
		List<Node> children = parentLayoutNode.getNode().getChildren();
		int k = children.size();
		for (int i = 0; i < children.size(); i++) {
	...
	private List<Float> getPercentageRatioTechnique3(KBLNode parentLayoutNode) {
		List<Float> childDimRatio = new Vector<Float>();
		List<Node> children = parentLayoutNode.getNode().getChildren();
		...
		for (int i = 0; i < children.size(); i++) {
			childCost.add(((InternalNode) parentLayoutNode.getNode())
					.getCost(i));
	...
	/*
	 * It calculates a vector, similar to the characteristic vector, as
	 * introduced by Golin '98
	 */
	...
	private List<Float> scalingTechnique2Or3(KBLNode parentLayoutNode,
			int flag, int technique) {
		List<KBLNode> kblChildren = parentLayoutNode.getChildren();
		List<Node> children = parentLayoutNode.getNode().getChildren();

		int k = children.size();
		// calculate the width of the parent, i.e. the avaialble share of real
		// estate
		 ...
		 * percentageRatio.add((float)2/3); percentageRatio.add((float)1/3);
		 */

		List<Float> idealPercentageRatio = getIdealPercentageRatio(
				parentLayoutNode, technique);

		for (int i = 0; i < kblChildren.size(); i++) {
			KBLNode kblChild = kblChildren.get(i);
			float tmpChildDim = kblChild.getRequiredDimension(flag)
					/ MAX_PERCENTAGE * parentScreenDim;
			// childDim.add(tmpChildDim);
			childDimPercentage.add((tmpChildDim / parentDimension)
					* MAX_PERCENTAGE);

		}

		double percentageSum = 0.0;
		// Sum all the percentage width values to check if the total percentage
		// is greater than 100% or less, or
		// equal
		...
		if (percentageDiff > 0) {
			...
			for (int i = 0; i < childDimPercentage.size(); i++) {
				...
				if ((childDimPercentage.get(i)) < idealPercentage
						* MAX_PERCENTAGE) {
					double diff = idealPercentage * MAX_PERCENTAGE
							- childDimPercentage.get(i);
					sumDiff += diff;
					childDiff.add(diff);
				} else {
					childDiff.add(0.0);
			...
			for (int i = 0; i < childDimPercentage.size(); i++) {
				double idealPercentage = idealPercentageRatio.get(i);
				if ((childDimPercentage.get(i) * MAX_PERCENTAGE) > idealPercentage
						* MAX_PERCENTAGE) {
					double diff = idealPercentage * 100
							- (childDimPercentage.get(i) * MAX_PERCENTAGE);
			...
			// We calcualte what is the percentage that should be subtracted
			// from each child's width
			// percentage to make sure the total width percentage remains ==
			// 100, i.e the child width
		...
		// All the above lines of code just calculated the percentage of width
		// which each child should be given,
		// in order to make sure that each button would be visible on screen,
		// and no button would have width
		// smaller than Min-width.
		for (int i = 0; i < childDimPercentage.size(); i++) {
			double tmp = (childDimPercentage.get(i) + (Math.abs(percentageDiff) * childDiff
					.get(i)))
					/ MAX_PERCENTAGE;
	...
	private List<Float> scalingTechnique3(KBLNode parentLayoutNode, int flag,
			int technique) {
		List<KBLNode> kblChildren = parentLayoutNode.getChildren();
		List<Node> children = parentLayoutNode.getNode().getChildren();
		...
		for (int i = 0; i < kblChildren.size(); i++) {
			...
			float tmpChildDim = kblChild.getRequiredDimension(flag)
					/ percentageSum;
			childDimPercentage.add(tmpChildDim);

		}
		return childDimPercentage;
	}

	private void configureBoxContainer(JPanel boxContainer, KBLNode layoutNode) {
		Node parentNode = layoutNode.getNode();
	...
	private JIndirectSelectionButton getButton(KBLNode layoutNode) {
		Node parentNode = layoutNode.getNode();
		...
		JIndirectSelectionButton button = (JIndirectSelectionButton) ((Node) parentNode)
				.getRepresentative();
	...
	private void createSpecialButtonForDigit(KBLNode kblDigit, Node digitNode) {

		MyTraversableEncodingTree digitCH = createDigitCH(digitNode);
	}

	private MyTraversableEncodingTree createDigitCH(Node digitNode) {
		InternalNode root = new InternalNode();

		LeafNode node1 = new LeafNode(JIndirectSelectionButton.VK_1);
		LeafNode node2 = new LeafNode(JIndirectSelectionButton.VK_2);
		LeafNode node3 = new LeafNode(JIndirectSelectionButton.VK_3);
		LeafNode node4 = new LeafNode(JIndirectSelectionButton.VK_4);
		LeafNode node5 = new LeafNode(JIndirectSelectionButton.VK_5);
		LeafNode node6 = new LeafNode(JIndirectSelectionButton.VK_6);
		LeafNode node7 = new LeafNode(JIndirectSelectionButton.VK_7);
		LeafNode node8 = new LeafNode(JIndirectSelectionButton.VK_8);
		LeafNode node9 = new LeafNode(JIndirectSelectionButton.VK_9);
		LeafNode node0 = new LeafNode(JIndirectSelectionButton.VK_0);

		InternalNode node12 = new InternalNode();
		node12.addChild(node1);
		node12.addChild(node2);

		InternalNode node45 = new InternalNode();
		node45.addChild(node4);
		node45.addChild(node5);

		InternalNode node345 = new InternalNode();
		node345.addChild(node3);
		node345.addChild(node45);

		InternalNode node12345 = new InternalNode();

		node12345.addChild(node12);
		node12345.addChild(node345);

		InternalNode node67 = new InternalNode();
		node67.addChild(node6);
		node67.addChild(node7);

		InternalNode node90 = new InternalNode();
		node90.addChild(node9);
		node90.addChild(node0);

		InternalNode node890 = new InternalNode();
		node890.addChild(node8);
		node890.addChild(node90);

		InternalNode node67890 = new InternalNode();
		...
		return new MyTraversableEncodingTree(root);
	}
	 ...
	 */
	public List getAllEnabledButtons() {
		List allButtons = new Vector();

		for (JIndirectSelectionButton but : keyList
