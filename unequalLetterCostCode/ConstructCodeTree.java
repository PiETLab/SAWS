...
public class ConstructCodeTree<X extends SourceSymbol> {
	 ...
	 * lexicographical linear order it returns -1 if entry 1 < entry 2 it
	...
	public ConstructCodeTree(List<Double> probabilities,
			List<Integer> costList, int n, int r) {
		...
		// level[1] = 7 would mean level 1 starts from the 7th index of
		// opt-table
		...
		while (construct) {
			...
			levels.add(size);
			// System.out.println("In while for level " + levelIndex + " size :
			// " + size);
			...
			for (int index = levels.get(levelIndex); index < size; index++) {
				...
				double newCost = OPT_Table.getValue(index)
						+ summationProbability(entry.getM());

				if (entry.getM() < n && !(entry.getSignature()).allZeros()) {

					for (int q = 0; q <= entry.getL1(); q++) {
						Signature expandedSig = (entry.getSignature())
								.calculateExpandedSignature(q, vectorD);
						if (expandedSig.sumAll() <= (n * (r - 1))) {
							...
							if (exists == -1) {
								...
								if (oldCost > newCost) {
									...
									searchAndUpdateEntriesWithLink(exists,
											oldCost, newCost);
									// for Debug
									 ...
									 * newEntry.debugLinks =
									 * oldEntry.debugLinks;
									 * newEntry.debugSigLevel =
									 * oldEntry.debugSigLevel;
									 * 
									 ...
									 */
									newEntry
											.setTerminal(oldEntry.getTerminal());
									newEntry.setQ(q);
									newEntry.setLinkIndex(index);
								...
								/*
								 * expandedSig.setLevel(levelIndex); double
								 * oldCost = OPT_Table.getValue(exists);
								 * TableEntry newEntry = new
								 * TableEntry(expandedSig,
								 * min(oldCost,newCost)); newEntry.setQ(q);
								 * newEntry.setLinkIndex(index);
								 * OPT_Table.set(exists, newEntry);
								 */
							}
	...
	public void searchAndUpdateEntriesWithLink(int link, double oldCost,
			double newCost) {
	...
	public static int calculateNumOfCj(int j, List<Integer> costList) {
		int result = 0;
	...
	private static Comparator<TableEntry> c = new Comparator<TableEntry>() {
		public int compare(TableEntry entry1, TableEntry entry2) {
			int result = (entry1.getSignature()).partialSumCompare(entry2
					.getSignature());
			return result;
		}
	};

	public InternalNode constructCodeforUnequalLetterCosts(
			EncodingAlphabet encodingAlphabet, List<SourceSymbol> enabledWords) {
		// System.out.println("------------Printing Code Not ---------------");

		// this.toString();

		List<JIndirectSelectionButton> encodingAlphabetSymbols = encodingAlphabet.getAlphabetList();
		List<Integer> encodingAlphabetCosts = encodingAlphabet
				.getAlphabetCostList();
		InternalNode codeTree = this.buildMinCostTree(enabledWords,
				encodingAlphabetCosts);
		/*
		 ...
		 */
		InternalNode codeTreeWithWords = this.assignActualProbabilities(
				codeTree, enabledWords);
	...
	public InternalNode buildMinCostTree(List<SourceSymbol> rChildSymbols,
		...
		if (index == 0) {
			...
			// System.out.println("in index 0 index:" + index +
			// OPT_Table.get(index));
		}
		...
		for (int j = (treeEntries.size() - 1); j >= 0; j--) {
			...
			if (sigLevel > 0) {
				if (tempEntry.getQ() > 0)
					// System.out.println("expanding " + tempEntry + " q" +
					// tempEntry.getQ());
					root = root.expandTreeAtLevelI(sigLevel, tempEntry.getQ(),
							rChildSymbols, rChildCosts);
	...
	/*
	 * It is assumed that this method is receiving as input the root node of the
	 * tree, to which we want to assignt probabilities
	 */

	public InternalNode assignActualProbabilities(InternalNode root,
			List<SourceSymbol> wordList) {
		int wordIndex = 0;
		int depth = root.calculateMaximumLeafDepth(root.getEdgeCostToHere());
		for (int i = 0; i <= depth; i++) {
			List<Node> leaves = root.getleavesAtLevel(i);
			for (Node prevLeaf : leaves) {
				if (wordIndex >= 0 && wordIndex < wordList.size()) {
					LeafNode newLeaf = new LeafNode(wordList.get(wordIndex));
					newLeaf.setDepth(prevLeaf.getEdgeCostToHere());
					// this line might be wrong
					wordIndex++;
					newLeaf.setEncodingAlphabet((prevLeaf.getRepresentative())
							.toString());

					List<Node> v = (prevLeaf.parent).getChildren();

					int childNum = v.indexOf(prevLeaf);
					((InternalNode) prevLeaf.parent)
							.setChildToParent(((InternalNode) prevLeaf.parent),
									newLeaf, childNum);
					// return wordIndex;
					// int temp = 0;
	...
	private InternalNode removeExtraInternalNodes(InternalNode node) {
		// InternalNode<X> node = node1.clone();
		int depth = node.calculateMaximumLeafDepth(node.getEdgeCostToHere());
		for (int i = depth; i >= 0; i--) {
			List<InternalNode> internalNodes = node.getInternalNodesAtLevel(i);
			for (InternalNode prevNode : internalNodes) {
				List<Node> children = prevNode.getChildren();
				if (children.size() == 0) {
					List<Node> v = (prevNode.parent).getChildren();
	 ...
	 * InternalNode<X> temp = root.clone(); subCleanStrayInternalNodes(temp);
	 * return temp; }
	 * 
	 * private void subCleanStrayInternalNodes(Node<X> root) {
	 * if(!root.isLeaf()) { List<Node<X>> children = root.getChildren(); if(
	 * children.size() == 0) { List<Node<X>> v = (root.parent).getChildren();
	 * 
	 * int childNum = v.indexOf(root); v.remove(childNum); return; } else {
	 * 
	 * for(int i = 0; i < children.size(); i ++) { Node<X> child =
	 * children.get(i); subCleanStrayInternalNodes(child); } } } else { return; } }
	 */
	private int subAssignProbabilities(Node node, List<X> wordList,
			int wordIndex) {
		if (node.isLeaf()) {
			if (wordIndex >= 0 && wordIndex < wordList.size()) {
				LeafNode leaf = new LeafNode(wordList.get(wordIndex));

				// this line might be wrong
				wordIndex++;
				leaf.setEncodingAlphabet((node.getRepresentative()).toString());

				List<Node> v = (node.parent).getChildren();

				int childNum = v.indexOf(node);
				((InternalNode) node.parent).setChildToParent(
						((InternalNode) node.parent), leaf, childNum);
				return wordIndex;
			} else if (wordIndex >= wordList.size()) {
				List<Node> v = (node.parent).getChildren();
	...
	/*
	 * private int subAssignProbabilities(Node<X> node, List<X> wordList, int
	 * wordIndex ) { if(node.isLeaf() ) { if(wordIndex >=0 && wordIndex <
	 * wordList.size()) { LeafNode<X> leaf = new LeafNode<X>(wordList.get(wordIndex));
	 * 
	 * //this line might be wrong wordIndex++;
	 * leaf.setEncodingAlphabet((node.getRepresentative()).toString());
	 * 
	 * List<Node<X>> v = (node.parent).getChildren();
	 * 
	 * int childNum = v.indexOf(node); ( (InternalNode<X>)node.parent).setChildToParent((
	 * (InternalNode<X>)node.parent), leaf, childNum); return wordIndex; } else
	 * if (wordIndex >=wordList.size()) { List<Node<X>> v =
	 * (node.parent).getChildren();
	 * 
	 * int childNum = v.indexOf(node); v.remove(childNum); return wordIndex; } }
	 * else { List<Node<X>> children = node.getChildren(); int wOld =
	 * wordIndex; int wNew = 0; for(Node<X> ch : children) {
	 * 
	 * wNew = subAssignProbabilities(ch, wordList, wOld ); wOld = wNew; } return
	 * wNew; }
	 * 
	 * return -1; }
	 */
	 ...
	 */
	/*
	 * public InternalNode<X> buildMinCostTree2(EncodingAlphabet alphabet, int
	 * k) { if(k > 0 && k <= alphabet.size()) {
	 * 
	 * List<Integer> encodingAlphabetCosts = new Vector<Integer>(); List<JVirtualKeyboardButton>
	 * alphabetSymbols = new Vector<JVirtualKeyboardButton> ();
	 * 
	 * for(int i = 0; i < k; i ++ ) {
	 ...
	 * 
	 * 
	 * int index = searchMinCostTree(); List <TableEntry> treeEntries = new
	 * Vector<TableEntry>(); while(index != 0) { TableEntry tempEntry=
	 * OPT_Table.get(index); treeEntries.add(tempEntry); index =
	 * tempEntry.getLinkIndex(); } if(index == 0) {
	 * treeEntries.add(OPT_Table.get(index)); }
	 * 
	 * InternalNode<X> root = new InternalNode<X>(); root =
	 * root.getTreeAtLevel0(alphabetSymbols, encodingAlphabetCosts);
	 * 
	 * for(int j = (treeEntries.size()-1); j >=0 ; j--) { TableEntry tempEntry =
	 * treeEntries.get(j); int sigLevel = tempEntry.getSignature().getLevel();
	 * if( sigLevel > 0 ) { if(tempEntry.getQ() > 0) root =
	 * root.expandTreeAtLevelI(sigLevel, tempEntry.getQ(), alphabetSymbols,
	 * encodingAlphabetCosts); } } root = root.updateAssociatedNodeSet();
	 * 
	...
	/*
	 * Calculate summation of probailities p(t) for m < t <= n our probabilites
	 * start from p[1] amd so on.... p[0] contains nothing
	 */
