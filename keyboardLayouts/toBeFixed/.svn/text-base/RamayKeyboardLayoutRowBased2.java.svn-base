...
public class RamayKeyboardLayoutRowBased2 extends ButtonLayoutSpecification {
	...
	public RamayKeyboardLayoutRowBased2(
			MyTraversableEncodingTree<JIndirectSelectionButton> root,
		...
		// InternalNode<JVirtualKeyboardButton> temp =
		// (InternalNode)root.getRoot();
		...
		// TextCompositionFacilityFrame.SCREEN_WIDTH);
		// System.out.println("Screen height" +
		// TextCompositionFacilityFrame.SCREEN_HEIGHT);
		...
		Graph.constructGraph(allButtons, "RowBased2_Technique3_Aug29.txt");
	...
	public KBLNode<JIndirectSelectionButton> preCalculationRowBased2(
			KBLNode<JIndirectSelectionButton> parentLayoutNode) {
		...
		float level = (float) Math.log(SCREEN_HEIGHT / MIN_BUTTON_HEIGHT)
				/ (float) Math.log(k);
		...
		if (maxLevel != -1) {
			if (level >= maxLevel) {
				 ...
				 * if(SCREEN_WIDTH/max >= MIN_BUTTON_WIDTH) { return
				 * preKBLCalculationRowBased1(parentLayoutNode); } else {
				 * maxLevel = (int)level; //} return
				 * preKBLCalculationRowBased2(parentLayoutNode, 0, maxLevel); }
				 */
	...
	public KBLNode<JIndirectSelectionButton> preCalculationRowBased3(
			KBLNode<JIndirectSelectionButton> parentLayoutNode) {
		...
		int maxDepth = ((InternalNode<JIndirectSelectionButton>) parentNode)
				.calculateMaximumLeafDepth(0);
		// This is a list that determines the size of the button by each level.
		for (int i = 0; i <= maxDepth; i++) {
			int num = (((InternalNode<JIndirectSelectionButton>) parentNode)
					.getleavesAtLevel(i)).size();
		...
		float minWidthPercentage = (MIN_BUTTON_WIDTH / SCREEN_WIDTH)
				* MAX_PERCENTAGE;
		float minHeightPercentage = (MIN_BUTTON_HEIGHT / SCREEN_HEIGHT)
				* MAX_PERCENTAGE;
		...
		return preKBLCalculationRowBased3(parentLayoutNode, numOfNodesAtDepth,
				widthPercentageByLevel, heightPercentageByLevel);
	}

	public KBLNode<JIndirectSelectionButton> preKBLCalculationRowBased3(
			KBLNode<JIndirectSelectionButton> parentLayoutNode,
			List<Integer> numOfNodesAtDepth,
			List<Float> widthPercentageByLevel,
			List<Float> heightPercentageByLevel) {
		Node<JIndirectSelectionButton> parentNode = parentLayoutNode.getNode();
		List<Node<JIndirectSelectionButton>> children = parentNode.getChildren();
		...
		if (parentNode.isLeaf()) {
			...
			if (parentLayoutNode.getDivideType() == KBLNode.VERTICAL) {
				...
				for (int i = 0; i < children.size(); i++) {
					...
					KBLNode<JIndirectSelectionButton> tmpChildNode = preKBLCalculationRowBased3(
							newChildNode, numOfNodesAtDepth,
							widthPercentageByLevel, heightPercentageByLevel);
					if (i == 0) {
				...
				for (int i = 0; i < children.size(); i++) {
					...
					int associatedNodeSize = ((child.getSelectionGroup())
							.extractMembers()).size();
					float prevWidth = 0;
					float childHeight = tmpChildNode.getRequiredHeight();
					// float childWidth = tmpChildNode.getRequiredWidth();
					float minButtonPercentage = (MIN_BUTTON_HEIGHT / SCREEN_HEIGHT)
							* MAX_PERCENTAGE;
					float associatedNodePercentage = (SCREEN_WIDTH / (float) associatedNodeSize);

					if (childHeight < minButtonPercentage
							|| associatedNodePercentage >= MIN_BUTTON_WIDTH) {
						newChildNode = new KBLNode<JIndirectSelectionButton>(
								child);
						newChildNode.setDivideType(KBLNode.VERTICAL);
						tmpChildNode = preKBLCalculationRowBased3(newChildNode,
								numOfNodesAtDepth, widthPercentageByLevel,
								heightPercentageByLevel);
					}
	...
	/*
	 * public KBLNode<JVirtualKeyboardButton> preKBLCalculation (KBLNode<JVirtualKeyboardButton>
	 * parentLayoutNode, int level, int maxLevel) { Node<JVirtualKeyboardButton>
	 * parentNode = parentLayoutNode.getNode(); List <Node<JVirtualKeyboardButton>>
	 * children = parentNode.getChildren();
	 * 
	 * if(parentNode.isLeaf() ) { if(level >= maxLevel) { float tmpW =
	 * (float)MIN_BUTTON_WIDTH/SCREEN_WIDTH; float tmpH = (float)
	 * MIN_BUTTON_HEIGHT/SCREEN_HEIGHT; float leafWidthPercentage = tmpW *
	 * MAX_PERCENTAGE; float leafHeightPercentage = tmpH * MAX_PERCENTAGE;
	 * 
	 * 
	 * parentLayoutNode.setRequiredWidth(leafWidthPercentage);
	 * parentLayoutNode.setRequiredHeight(leafHeightPercentage); } else {
	 ...
	 *  } else { if (parentLayoutNode.getDivideType() == KBLNode.VERTICAL) { int
	 * k = children.size(); level = level +1; float maxHeight = 0; //All the
	 * children are assigned the avalaible percentages of the width for (int i =
	 * 0; i < children.size(); i++) { Node<JVirtualKeyboardButton> child =
	 * children.get(i);
	 * 
	 ...
	 * 
	 * if(parentLayoutNode.getAvaialbleWidth() != -1) { float childWidth =
	 * (float)childPercentage * parentLayoutNode.getAvaialbleWidth();
	 * 
	 * newChildNode.setDivideType(KBLNode.HORIZONTAL);
	 * newChildNode.setAvaialbleWidth(childWidth);
	 * newChildNode.setAvaialbleHeight(parentLayoutNode.getAvaialbleHeight());
	 * //level = level +1; KBLNode<JVirtualKeyboardButton> tmpChildNode =
	 * preKBLCalculation(newChildNode,level, maxLevel); if(i == 0) {
	 ...
	 * else { float tmp = parentLayoutNode.getRequiredWidth();
	 * parentLayoutNode.setRequiredWidth(tmp + tmpChildNode.getRequiredWidth());
	 * //parentLayoutNode.setRequiredHeight(tmpChildNode.getRequiredHeight()); }
	 * if (tmpChildNode.getRequiredHeight() > maxHeight) { maxHeight =
	 * tmpChildNode.getRequiredHeight(); }
	 * //parentLayoutNode.setRequiredHeight(parentLayoutNode.getAvaialbleHeight());
	 * parentLayoutNode.setRequiredHeight(maxHeight);
	 * parentLayoutNode.addChild(tmpChildNode); } } } else { int k =
	 * children.size(); level = level +1; float maxWidth = 0; //All the children
	 * are assigned the avalaible percentages of the width for (int i = 0; i <
	 * children.size(); i++) { Node<JVirtualKeyboardButton> child =
	 * children.get(i);
	 * 
	 * double childPercentage = 1.0/(double)k;
	 * 
	 * KBLNode<JVirtualKeyboardButton> newChildNode = new KBLNode<JVirtualKeyboardButton>(child);
	 * 
	 * if(parentLayoutNode.getAvaialbleHeight() != -1) { float childHeight =
	 * (float)childPercentage * parentLayoutNode.getAvaialbleHeight();
	 * 
	 * newChildNode.setDivideType(KBLNode.VERTICAL);
	 * newChildNode.setAvaialbleHeight(childHeight);
	 * newChildNode.setAvaialbleWidth(parentLayoutNode.getAvaialbleWidth());
	 * KBLNode<JVirtualKeyboardButton> tmpChildNode =
	 * preKBLCalculation(newChildNode,level, maxLevel); if(i == 0) {
	 * parentLayoutNode.setRequiredHeight((tmpChildNode).getRequiredHeight());
	 * //parentLayoutNode.setRequiredWidth((tmpChildNode).getRequiredWidth()); }
	 * else { float tmp = parentLayoutNode.getRequiredHeight();
	 * parentLayoutNode.setRequiredHeight(tmp +
	 * (tmpChildNode).getRequiredHeight());
	 * //parentLayoutNode.setRequiredWidth((tmpChildNode).getRequiredWidth()); }
	 * //parentLayoutNode.setRequiredWidth(parentLayoutNode.getAvaialbleWidth());
	 * 
	 * if (tmpChildNode.getRequiredWidth() > maxWidth) { maxWidth =
	 * tmpChildNode.getRequiredWidth(); }
	 * parentLayoutNode.setRequiredWidth(maxWidth);
	 * parentLayoutNode.addChild(tmpChildNode); } }
	 *  } } return parentLayoutNode; }
	 */
	public KBLNode<JIndirectSelectionButton> preKBLCalculationRowBased2(
			KBLNode<JIndirectSelectionButton> parentLayoutNode, int level,
			int maxLevel) {
		Node<JIndirectSelectionButton> parentNode = parentLayoutNode.getNode();
		List<Node<JIndirectSelectionButton>> children = parentNode.getChildren();
				...
				for (int i = 0; i < children.size(); i++) {
					...
					if (parentLayoutNode.getAvaialbleWidth() != -1) {
						...
						newChildNode.setAvaialbleHeight(parentLayoutNode
								.getAvaialbleHeight());
						// level = level +1;
						KBLNode<JIndirectSelectionButton> tmpChildNode = preKBLCalculationRowBased2(
								newChildNode, level, maxLevel);
						if (i == 0) {
							parentLayoutNode.setRequiredWidth(tmpChildNode
									.getRequiredWidth());
							// parentLayoutNode.setRequiredHeight(tmpChildNode.getRequiredHeight());
				...
				for (int i = 0; i < children.size(); i++) {
					...
					if (parentLayoutNode.getAvaialbleHeight() != -1) {
						...
						if (level == maxLevel) {
							...
							float tmpWidthPercent = newChildNode
									.getAvaialbleWidth();
							Node<JIndirectSelectionButton> tmpChildNode = newChildNode
									.getNode();
							int numOfAssociateNodes = tmpChildNode
									.getSelectionGroupSize();

							float widthRatio = tmpWidthPercent
									/ (float) numOfAssociateNodes;
							float minScreenRatio = (float) MIN_BUTTON_WIDTH
									/ SCREEN_WIDTH * MAX_PERCENTAGE;
	...
	public JPanel CHToKBLRowBased(
		...
		if (parentNode.isLeaf()) {
			...
			if (parentLayoutNode.getDivideType() == KBLNode.VERTICAL) {
				...
				float parentWidth = parentLayoutNode.getEndX()
						- parentLayoutNode.getStartX();
				// List<JVirtualKeyboardButton> buttonList =
				// ((parentLayoutNode.getNode()).getNodeSelectionGroup()).extractMembers()
				...
				for (int i = 0; i < kblChildren.size(); i++) {
					...
					if (childHeight < MIN_BUTTON_HEIGHT
							|| (childHeight <= 2 * MIN_BUTTON_HEIGHT && (childWidth / (float) associatedNodeSize) >= MIN_BUTTON_WIDTH)) {
	...
	private void setChildLayoutNodeRowBased(
			KBLNode<JIndirectSelectionButton> parentLayoutNode,
	...
	private List<Float> calculateWidthPercentageRowBased3(
		...
		List<Float> dimPercentageDepthBased = scalingTechnique2Or3(
				parentLayoutNode, flag, technique);
		List<Float> idealDimPercentage = getIdealPercentageRatio(
		...
		return calculateDimensionPercentageRowBased1Or2(parentLayoutNode, flag,
				technique);
	}
	...
	private List<Float> calculateDimensionPercentageRowBased(
		...
		if (technique == TECHNIQUE1) {
			...
			return calculateWidthPercentageRowBased3(parentLayoutNode, flag,
					technique);
		}
		return calculateDimensionPercentageRowBased1Or2(parentLayoutNode, flag,
				technique);
	...
	private List<Float> calculateDimensionPercentageRowBased1Or2(
			KBLNode<JIndirectSelectionButton> parentLayoutNode, int flag,
			int technique) {
		List<KBLNode<JIndirectSelectionButton>> kblChildren = parentLayoutNode
				.getChildren();
	 ...
	 *  } else if (flag == KBLNode.HEIGHT) { parentDimension = parentHeight;
	 ...
	 *  } //calculate the percentage difference double percentageDiff =
	 * (MAX_PERCENTAGE-percentageSum); Vector<Double> childDiff = new Vector<Double>();
	 * double sumDiff = 0.0; if(percentageDiff > 0) { sumDiff = 0.0; for (int i =
	 * 0; i < childDimPercentage.size(); i++) { double idealPercentage = 1.0/k;
	 ...
	 *  } else if(percentageDiff < 0) { sumDiff = 0.0; for (int i = 0; i <
	...
	private List<Float> getIdealPercentageRatio(
			KBLNode<JIndirectSelectionButton> parentLayoutNode, int technique) {
	...
	private List<Float> getPercentageRatioTechnique2(
			KBLNode<JIndirectSelectionButton> parentLayoutNode) {
		List<Float> childDimRatio = new Vector<Float>();
		List<Node<JIndirectSelectionButton>> children = parentLayoutNode
				.getNode().getChildren();
	...
	private List<Float> getPercentageRatioTechnique3(
			KBLNode<JIndirectSelectionButton> parentLayoutNode) {
		List<Float> childDimRatio = new Vector<Float>();
		List<Node<JIndirectSelectionButton>> children = parentLayoutNode
				.getNode().getChildren();
		...
		for (int i = 0; i < children.size(); i++) {
			childCost
					.add(((InternalNode<JIndirectSelectionButton>) parentLayoutNode
							.getNode()).getCost(i));
	...
	private List<Float> scalingTechnique2Or3(
			KBLNode<JIndirectSelectionButton> parentLayoutNode, int flag,
			int technique) {
		List<KBLNode<JIndirectSelectionButton>> kblChildren = parentLayoutNode
				.getChildren();
	...
	/*
	 * private void configureBoxContainerRowBased(JPanel boxContainer, KBLNode<JVirtualKeyboardButton>
	 * layoutNode) { Node<JVirtualKeyboardButton> parentNode =
	 * layoutNode.getNode();
	 * 
	 * if (parentNode.isLeaf()) { boxContainer.setLayout(new
	 * BoxLayout(boxContainer, BoxLayout.PAGE_AXIS)); JVirtualKeyboardButton
	 * button = this.getButton(layoutNode); boxContainer.add(button); } else {
	 * if(layoutNode.getDivideType() == KBLNode.VERTICAL) {//parent node
	 * Vertical //JPanel subBoxContainer = CHToKBL (layoutNode);
	 * 
	 * //this.configureBoxContainer(boxContainer, parentLayoutNode); //return
	 * boxContainer; boxContainer.setLayout(new BoxLayout(boxContainer,
	 * BoxLayout.LINE_AXIS)); //boxContainer.add(subBoxContainer);
	...
	private MyTraversableEncodingTree<JIndirectSelectionButton> createDigitCH(
			Node<JIndirectSelectionButton> digitNode) {
