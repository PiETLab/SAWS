...
public class RowBasedKeyboardLayoutLevelParameterized2 extends ButtonLayoutSpecification {
	...
	public RowBasedKeyboardLayoutLevelParameterized2(
	...
	public JPanel CHToKBL(KBLNode<JIndirectSelectionButton> parentLayoutNode) {
		...
		if (parentNode.isLeaf()) {
			...
			if (parentLayoutNode.getDivideType() == KBLNode.VERTICAL) {
				...
				// calculate the width of the parent, i.e. the avaialble
				...
				for (int i = 0; i < children.size(); i++) {
					...
					if (maxLevel != -1) {
						// maxLevel +=1;
						// calculate the minimum width needed by each child,
						// in order to make sure that each leaf node
						// at the maxLevel, gets the required
						// MIN_BUTTON_WIDTH.
						minChildWidth = MIN_BUTTON_WIDTH
								* (float) Math.pow(k, Math
										.ceil((double) maxLevel / k));

					} else {
						minChildWidth = MIN_BUTTON_WIDTH;
					}
					// store all the percentage width values calculated for
					// the children for future use.
					childPercentage.add((double) minChildWidth / parentWidth);
				}
				double percentageSum = 0.0;
				// Sum all the percentage width values to check if the total
				// percentage is greater than 100% or less, or
				// equal
				...
				double sumDiff = 0.0;
				// if percentage difference is > 0, it means if all the
				// child nodes are assigned the min-width that they
				// need, then some of the real estate share of the parent
				// width would still be left, and it needs to be
				// divided among the children.
				if (percentageDiff > 0) {
					sumDiff = 0.0;
					for (int i = 0; i < childPercentage.size(); i++) {
						// This is the ideal percentage of parent width that
						// each child should be assigned,
						// if there were no limitations i.e. the child nodes
						// do not become very small,
						// and hard to read in the deepest levels.
						double idealPercentage = 1.0 / k;

						// If the child has less than its ideal share, than
						// this child would be later given an extra
						// share of percentage afterwards, from the
						// excessive percentage i.e. percentage difference

						if ((childPercentage.get(i) * 100) < idealPercentage * 100) {
							double diff = idealPercentage * 100
									- (childPercentage.get(i) * 100);
							sumDiff += diff;
							childDiff.add(diff);
						} else {
							childDiff.add(0.0);
					...
					// We calcualte what is the percentage of the excessive
					// percentage, that each child with less than ideal
				...
				// if percentage difference is < 0, it means that if all the
				// child nodes are
				// assigned the min-width that they
				// need, then the sum of thier percentages would become
				// greater than the real estate share of the parent width
				// that is available, and hence it is not possible- so the
				// percentages needed to be altered to fit within
				// the actual parent width
					...
					for (int i = 0; i < childPercentage.size(); i++) {
						...
						// we calculate the ideal percentage for this child,
						// according to
						// the original algorithm - this ideal percentage
						// however would result in smaller button sizes
					...
					// We calcualte what is the percentage that should be
					// subtracted from each child's width
					// percentage to make sure the total width percentage
					// remains == 100, i.e the child width
					// sum up tp the width of the parent, and is not more
					// that it.
				...
				// All the above lines of code just calculated the
				// percentage of width which each child should be given,
				// in order to make sure that each button would be visible
				// on screen, and no button would have width
				// smaller than Min-width.
				for (int i = 0; i < childPercentage.size(); i++) {
					double tmp = ((childPercentage.get(i) * 100) + (Math
							.abs(percentageDiff) * childDiff.get(i))) / 100;
					childPercentage.set(i, tmp);
				}
				int tmpSum = 0;

				// We calculate here if the child actually needs adjustment
				// in width percentage or not
				// may be this step is redundant... ? needs to be looked
				// into
				for (int i = 0; i < childPercentage.size(); i++) {
					double tmp1 = Math.round((childPercentage.get(i) * 100));
					double tmp2 = Math.round((1.0 / k) * 100);
					if (Math.abs(tmp1 - tmp2) <= pixelMargin)
					// if (Math.round(childPercentage.get(i)* 100) ==
					// (buttonWidth/k));
					{
						tmpSum++;
					}
				...
				// All the children are assigned the percentages of the
				// width
				for (int i = 0; i < children.size(); i++) {
					Node<JIndirectSelectionButton> child = children.get(i);
					...
					KBLNode<JIndirectSelectionButton> newChildNode = new KBLNode<JIndirectSelectionButton>(
							child);
					parentWidth = parentLayoutNode.getEndX()
							- parentLayoutNode.getStartX();
					float interval = 0;
					...
					// New child node is created and dimensions are assigned
					// to it accordingly, and the
					// divide type is set.
				...
				if (k > parentHeight/ MIN_BUTTON_HEIGHT) {
					this.configureBoxContainer(boxContainer, parentLayoutNode);
				} else if (k <= parentHeight / MIN_BUTTON_HEIGHT) {
					// {
					boxContainer.setLayout(new BoxLayout(boxContainer,
							BoxLayout.PAGE_AXIS));
					...
					int totalInterval = 0;
					boolean adjustmentNeeded = false;
					// calculate the height of the parent, i.e. the available
					// share of real estate
					...
					for (int i = 0; i < children.size(); i++) {
						...
						if (maxLevel != -1) {
							// calculate the minimum height needed by each child
							// in order to make sure that each leaf node
							// at the maxLevel, gets the required
							// MIN_BUTTON_Height
						...
						//
						// store all the percentage height values calculated for
						// the children for future use.
						childPercentage.add((double) minChildHeight
								/ parentHeight);
					}
					double percentageSum = 0.0;

					// Sum all the percentage height values to check if the
					// total percentage
					// is greater than 100% or less, or equal
					for (int i = 0; i < childPercentage.size(); i++) {
						percentageSum += Math
								.ceil(childPercentage.get(i) * 100);
					...
					double sumDiff = 0.0;
					// if percentage difference is > 0, it means that if all the
					// child nodes are assigned the min-height that they
					// need, then some of the real estate share of the parent
					// height would still be left, and it needs to be
					// divided among the children.

					if (percentageDiff > 0) {
						for (int i = 0; i < childPercentage.size(); i++) {
							// //This is the ideal percentage of parent height
							// that each child should be assigned according to
							// the
							// original algorithm,
							// if there were no limitations i.e. the child nodes
							// do not become very small,
							...
							// If the child has less than its ideal share, than
							// this
							// child would be later
							// given an extra
							// share of percentage from the excessive percentage
					...
					// if percentage difference is < 0, it means that if all the
					// child nodes are
					// assigned the min-height that they
					// need, then the sum of their percentages would become
					// greater than the real estate share of the parent height
					// that is available, and hence it is not possible - so the
					// percentages needed to be altered to fit within
					// the actual parental height.
						...
						// We calcualte what is the percentage that should be
						// subtracted from each child's height
						// percentage to make sure the total height percentage
						// remains == 100, i.e the all children's height
						// sum up to the width of the parent, and is not more
						// than it.
					...
					// All the above lines of code just calculated the
					// percentage of height which each child should be given,
					// in order to make sure that each button would be visible
					// on screen, and no button would have height
					// smaller than Min-width.
					for (int i = 0; i < childPercentage.size(); i++) {
						double tmp = ((childPercentage.get(i) * 100) + (Math
								.abs(percentageDiff) * childDiff.get(i))) / 100;
					...
					for (int i = 0; i < children.size(); i++) {
						...
						parentHeight = parentLayoutNode.getEndY()
								- parentLayoutNode.getStartY();

						float interval = 0;
						interval = Math
								.round(parentHeight * nodeSizePercentage);
	...
	private void configureBoxContainer(JPanel boxContainer,
		...
		if (parentNode.isLeaf()) {
			...
			boxContainer.setLayout(new BoxLayout(boxContainer,
					BoxLayout.LINE_AXIS));
			List<Node<JIndirectSelectionButton>> children = parentNode
					.getChildren();
			System.out.println("." + children);

			// MB: not yet working
			// now we want the buttons arranged in a row // the following is not working
			for (Node<JIndirectSelectionButton> childLayoutNode : children) {
				JIndirectSelectionButton button = childLayoutNode.getRepresentative();
				boxContainer.add(button);
				System.out.println(childLayoutNode.getRepresentative());
			}
		}
	}
